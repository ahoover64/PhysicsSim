...
void init() {
  ...
  camera = new Camera(Vector3d(0, 5, 30), Vector3d(0, 5, 0),
  Vector3d(0, 1, 0));
  pc = new Emitter();
  pc->init();
  ...
  // ADDING STUFF
/*  Particle p;
  p.vel = Vector3d(15,15, -15);
  p.radius = 2.0;
  p.mass = 0.05;
  p.pos = Vector3d(0.0, 10.0, 0.0);
  pc->add_particle(p);

  Plane plane;
  plane.point = Vector3d(.001,0,0);
  plane.norm = Vector3d(0,1,0);
  plane.coef_friction = .1;
  plane.coef_restitution = .9;
  plane.width = 30.0;
  plane.height = 30.0;
  plane.color = Vector4d(1.0f,1.0f,0.0f,0.2f);
  pc->add_plane(plane);
  plane.point = Vector3d(15,15,0);
  plane.norm = Vector3d(-1,0,0);
  plane.color = Vector4d(0.0f,1.0f,0.0f,0.2f);
  pc->add_plane(plane);
  plane.point = Vector3d(-15,15,0);
  plane.norm = Vector3d(1,0,0);
  plane.color = Vector4d(1.0f,0.0f,0.0f,0.2f);
  pc->add_plane(plane);
  plane.point = Vector3d(0.001,30,0);
  plane.norm = Vector3d(0,-1,0);
  plane.color = Vector4d(0.0f,0.0f,1.0f,0.2f);
  pc->add_plane(plane);
  plane.point = Vector3d(0,15,15);
  plane.norm = Vector3d(0,0,-1);
  plane.color = Vector4d(0.0f,1.0f,1.0f,0.2f);
  pc->add_plane(plane);
  plane.point = Vector3d(0,15,-15);
  plane.norm = Vector3d(0,0,1);
  plane.color = Vector4d(0.0f,1.0f,0.0f,0.2f);
...
void PerspDisplay() {
  ...
  std::vector<Particle> particles = pc->live_particles();
  ...
  // Shade Planes magenta in lighting mode
  glColor4f(1.0f, 0.0f, 1.0f,0.5f);
  std::vector<Plane> planes = pc->planes();
  for(int i = 0; i < planes.size(); i++) {
    glColor4f(planes[i].color[0], planes[i].color[1], planes[i].color[2], planes[i].color[3]);
    draw_plane(planes[i]);
  }
  //glLoadIdentity();
  glutSwapBuffers();
...
int main(int argc, char *argv[]) {
  ...
  //std::vector<Particle> parts = pc->particles();
  //for(int i = 0; i < parts.size(); i++) {
  //  std::cout << parts[i].pos << std::endl;
  //}
